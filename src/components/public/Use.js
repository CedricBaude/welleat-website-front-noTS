...
const Use = () => {
    ...
    const carouselLogo = (postLogo?.data || []).map((data) => <CarouselLogo id_prod={data?.id} url={data?.lien} />);
    ...
    return (
        <section className='sub-content use' id='returns'>
            ...
            <Carousel responsive={responsive} showDots={false} autoPlay={true} autoPlaySpeed={2000} infinite={true}>{carouselLogo}</Carousel>
